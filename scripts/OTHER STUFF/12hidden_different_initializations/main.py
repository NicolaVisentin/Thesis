...
def Loss(
    ...
    robot : PlanarPCS_simple
        Robot instance.
    controller : MLP
        "Dummy" instance of the controller (actual parameters are in params_optimiz) tau = MLP(z, u).
    s_thresh : float
    ...
    # ...controller
    controller_updated = mlp_controller.update_params(p_controller)
    ...
    tau_batch = controller_updated.forward_batch(contr_inp)
...
dataset = onp.load(dataset_folder/'soft robot optimization/MG_RON_N12_DT0.15/dataset_m1e5_N12_DT0.15.npz')
...
if train_samples:
    # Epochs and batches
    n_epochs = n_epochs_samples # number of epochs
    ...
    # Epochs and batches
    n_epochs = n_epochs_long # number of epochs
    batch_size = 2**6        # batch size
...
if train_samples:
    ...
    p_robot0 = (L0_raw, D0_raw, r0_raw, rho0_raw, E0_raw, G0_raw)
    ...
    mlp_controller = MLP(key=keyController, layer_sizes=mlp_sizes, scale_init=scale_init, last_layer=last_layer_activation) # dummy instance

    p_controller0 = mlp_controller.init_params_batch(keyController, n_samples)              # generates as many params as the number of samples
else:
    # Load data
    ...
    scale_init = 0.001
    last_layer_activation = 'linear'
    mlp_controller = MLP(key=subkey, layer_sizes=mlp_sizes, scale_init=scale_init, last_layer=last_layer_activation) # dummy instance
    all_controller_before = mlp_controller.load_params(data_folder/f'{load_case_prefix}_all_data_controller_before.npz')

    p_controller0 = mlp_controller.extract_params_from_batch(
...
params_optimiz0 = (p_robot0, p_map0, p_controller0)
...
for i in range(n_samples):
    params_i = mlp_controller.extract_params_from_batch(p_controller0, i)
    u = test_set["u"] # shape (testset_size, 1)
    ...
    contr_inp = jnp.concatenate([z, u], axis=1) # shape (testset_size, 2*3*n_pcs + 1)
    tau_i = mlp_controller._forward_batch(params_i, contr_inp) # shape (testset_size, 3*n_pcs)
...
p_robot_after, p_map_after, p_controller_after = params_optimiz_after

A_raw_after, c_after = p_map_after
L_raw_after, D_raw_after, r_raw_after, rho_raw_after, E_raw_after, G_raw_after = p_robot_after
...
powers_msv_after = []
for i in range(n_samples):
    params_i = mlp_controller.extract_params_from_batch(p_controller_after, i)
    u = test_set["u"] # shape (testset_size, 1)
...
mlp_controller._save_params(p_controller0, data_folder/'all_data_controller_before.npz')
onp.savez(
...
mlp_controller._save_params(p_controller_after, data_folder/'all_data_controller_after.npz') 
onp.savez(
