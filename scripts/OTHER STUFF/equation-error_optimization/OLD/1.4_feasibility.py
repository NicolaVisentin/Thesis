...
def A2Araw(A: Array, A_thresh: float=0.0) -> Array:
    """A is n-by-n lower triangular. A_raw is n(n+1)/2 vector: first n elements are main diagonal
    of A (with softplus parametrization), remaining elements are the lower-triangular elements in 
    ROW ORDER."""
    n = A.shape[0]                                     # A is n-by-n lower triangular
    diag_raw = InverseSoftplus(jnp.diag(A) - A_thresh) # extract and tranform main diagonal of A (!softplus)
    off_raw_list = []                                  # handle off diagonal elements
    for i in range(1, n):
        for j in range(i):
            off_raw_list.append(A[i, j])
    off_raw = jnp.array(off_raw_list)
    A_raw = jnp.concatenate([diag_raw, off_raw])
    return A_raw

# Converts A_raw -> A
@partial(jax.jit, static_argnums=(1,2))
def Araw2A(A_raw: Array, n: int, A_thresh: float=0.0) -> Array:
    """A is n-by-n lower triangular. A_raw is n(n+1)/2 vector: first n elements are main diagonal
    of A (with softplus parametrization), remaining elements are the lower-triangular elements in 
    ROW ORDER."""
    diag_raw = A_raw[:n] # main diagonal
    off_raw = A_raw[n:]  # off diagonal
    A = jnp.diag( A_thresh + jax.nn.softplus(diag_raw) ) # assign main diagonal
    idx = 0
    for i in range(1, n):
        for j in range(i):
            A = A.at[i, j].set(off_raw[idx])
            idx += 1
    return A
...
def Loss(
    ...
    # convert parameters
    A = Araw2A(A_raw, y_batch.shape[1], A_thresh)
...
# Extract first 3 decoupled oscillators
y = y[:,:3]
yd = yd[:,:3]
ydd = ydd[:,:3]
...
A_thresh = 0.0001
A0 = jnp.tril(0.5*jnp.ones((n_ron,n_ron)))
A0 = A0.at[0,0].set(15)
c0 = jnp.array([0.0, 0.25, 0.0])
L0 = jnp.array([1e-1])
...
if True:
    ...
    A_raw_opt, c_opt = Phi_opt
    L_raw_opt, D_raw_opt = phi_opt

    A_opt = Araw2A(A_raw_opt, n_ron, A_thresh)
