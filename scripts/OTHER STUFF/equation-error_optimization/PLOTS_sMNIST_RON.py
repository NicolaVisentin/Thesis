...
experiment = 'A5' # name of the experiment to perform/load
...
ron_dataset = 'sMNIST_RON_N6_DT0.006_RHO0.99/dataset_m1e5_N6_DT0.006_RHO0.99' # name of the case to load from 'soft robot optimization' folder
ron_evolution_example = 'sMNIST_RON_N6_DT0.006_RHO0.99/RON_evolution_N6_DT0.006_RHO0.99_long' # name of the case to load from 'soft robot optimization' folder
...
if show_simulations:
    ...
    # for i in range(n_show):
    #     axs[i].plot(timePCS, u_pcs[:,i], 'b')
    #     axs[i].grid(True)
    #     axs[i].set_xlabel('t [s]')
    #     if i == 0 or i == 3:
    #         axs[i].set_ylabel(r"$\tau_{be}$ [$N\cdot m^2$]")
    #     elif i == 1 or i == 4:
    #         axs[i].set_ylabel(r"$\tau_{ax}$ [$N\cdot m$]")
    #     else:
    #         axs[i].set_ylabel(r"$\tau_{sh}$ [$N\cdot m$]")
    #     axs[i].set_title(f'Component {i+1}')
    #     #axs[i].set_ylim([onp.min(y_RONsaved[:,i])-1, onp.max(y_RONsaved[:,i])+1])
