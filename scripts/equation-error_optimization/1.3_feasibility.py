...
def A2Araw(A: Array, A_thresh: float=0.0) -> Array:
    return InverseSoftplus(jnp.diag(A) - A_thresh)

# Converts A_raw -> A
@partial(jax.jit, static_argnums=(1,))
def Araw2A(A_raw: Array, A_thresh: float=0.0) -> Array:
    return jnp.diag( A_thresh + jax.nn.softplus(A_raw) )
...
def Loss(
    ...
    params_optimiz : Sequence
        ...
        - **Phi**: Tuple (A_raw, c). A_raw.shape=(3*n_pcs,)=(n_ron,), c.shape=(3*n_pcs,)
...
A_thresh = 0.0001
A0 = jnp.diag(jnp.array([15, 0.5, 0.5]))
