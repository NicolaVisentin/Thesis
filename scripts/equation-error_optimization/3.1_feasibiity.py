...
def Loss(
    ...
    robot, controller, encoder, decoder
        "Dummy" instances. Actual parameters are in params_optimiz.
    trajectory : Array
        Reference time-trajectory y(t) for additional part of the loss (just one single trajectory for a 
        quick test). Shape (n_timesteps, n_ron)
    init_vel : Array
        Initial velocity for the provided trajectory. Shape (n_ron,)
    saveat : Array
        Time vector for evaluating simulation values (t0 and tf must be consistent with "trajectory"). Shape (n_timesteps,)
    ...
    # Simulate robot
    q0, qd0 = encoder_updated.forward_xd(trajectory[0], init_vel)
    ...
    def tau_law(system_state: SystemState, controller: MLP):
        ...
        return tau, None
    tau_fb = jax.jit(partial(tau_law, controller=controller_updated)) # signature u(SystemState) -> (control_u, control_state_dot) required by soromox
    sim_out = robot_updated.rollout_closed_loop_to(
    ...
    q_PCS, _ = jnp.split(sim_out.y, 2, axis=1)
    y_ts_hat = decoder.forward_batch(q_PCS) # y_hat(t) = psi(q(t)). Shape (n_steps, n_ron)
    simulation_loss = 1e0 * jnp.mean(jnp.sum((y_ts_hat - trajectory)**2, axis=1))

    # complete loss
    loss = MSE + reconstruction_loss + simulation_loss
...
if show_simulations:
    ...
    # Simulate robot
    q0, qd0 = encoder.forward_xd(y_RONsaved[0], yd_RONsaved[0])
    initial_state_pcs = SystemState(t=t0, y=jnp.concatenate([q0, qd0]))
    ...
    y_hat_pcs, yd_hat_pcs = decoder.forward_xd_batch(q_PCS, qd_PCS) # y_hat(t) = psi(q(t)); yd_hat(t) = J_psi(q(t))*qd(t). Shape (n_steps, n_ron)
...
idx_end = int(len(y_RONsaved)/10) # take a portion 1/N of the full trajectory
t1_loss = time_RONsaved[idx_end]
saveat_loss = jnp.arange(0, t1_loss, 1e-2)
def interpolate_trajectory(t_old, traj_old, t_new):
    return jnp.stack([
        jnp.interp(t_new, t_old, traj_old[:, i])
        for i in range(traj_old.shape[1])
    ], axis=1)
trajectory = interpolate_trajectory(time_RONsaved[:idx_end], y_RONsaved[:idx_end], saveat_loss)
...
if show_simulations:
    ...
    # Simulate robot
    q0, qd0 = encoder_opt.forward_xd(y_RONsaved[0], yd_RONsaved[0])
    initial_state_pcs = SystemState(t=t0, y=jnp.concatenate([q0, qd0]))
    ...
    y_hat_pcs, yd_hat_pcs = decoder_opt.forward_xd_batch(q_PCS, qd_PCS) # y_hat(t) = psi(q(t)); yd_hat(t) = J_psi(q(t))*qd(t). Shape (n_steps, n_ron)
