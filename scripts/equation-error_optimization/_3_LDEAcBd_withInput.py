...
def Loss(
    ...
    Takes a batch of datapoints (y, yd, u), converts them into configurations (q, qd, u_pcs)
    of the robot as q = A*y + c, qd = A*yd, u_pcs = B*u + d, computes the forward dynamics qdd = forward_dynamics_robot(q, qd, u_pcs)
    ...
    params_optimiz : tuple
        ...
        - **E**: pcs segments elastic modulus. Array of shape (n_pcs,)
        ...
        - **B_flat**: Mapping matrix between RON and robot input: u_pcs = B*u + d. Shape (n_act_pcs*n_input,)
        - **d**: Mapping vector between RON and robot input: u_pcs = B*u + d. Shape (n_act_pcs,)
    data_batch : dict
    ...
    """
    # extract everything
    L_softplus, D_softplus, E, A_softplus, c, B_flat, d = params_optimiz
    y_batch, yd_batch, u_batch, ydd_batch = data_batch["y"], data_batch["yd"], data_batch["u"], data_batch["ydd"]

    n_pcs = int(len(c) / 3)
    n_ron = y_batch.shape[1]
    n_act_pcs = 3*n_pcs
    n_input = u_batch.shape[1]
    ...
    B = jnp.reshape(B_flat, (n_act_pcs,n_input))
    ...
    u_pcs_batch = u_batch @ jnp.transpose(B) + d # shape (batch_size, n_act_pcs)
    ...
    zd = forward_dynamics_vmap(0, z, (u_pcs_batch,)) # state derivative zd=[qd^T, qdd^T]. Shape (batch_size, 2*3*n_pcs)
...
dataset = onp.load(dataset_folder/'soft robot optimization/dataset_m1e5_N6_withInput.npz')
...
u = dataset["u"]     # input of the RON oscillators. Shape (m, n_input)
...
params_optimiz = (L_softplus, D_softplus, E, A_softplus, c, B, d)
...
if show_simulations:
    ...
    RON_evolution_data = onp.load(saved_data_folder/'RON_evolution_N6_withInput.npz')
    ...
    for i, ax in enumerate(axs.flatten()):
        ...
        ax.set_xlabel('t [s]')
        ax.set_ylabel(r'$u_{PCS}$')
        ax.set_title(f'Component {i+1}')
        lines = [u_pcs_line, u_line]
        labels = [r'$u_{PCS}(t)$', r'$u_{RON}(t)$']
        ax.legend(lines, labels)
    plt.tight_layout()
    plt.savefig(plots_folder/'RONvsPCS_input_before', bbox_inches='tight')
    #plt.show()
...
print(f'Example:\n'
      f'    (y, yd, u) = ({onp.array(test_set["y"][69])}, {onp.array(test_set["yd"][69])}, {onp.array(test_set["u"][69])})\n'
      f'    (q, qd, u_pcs) = (A*y+c, A*yd, B*u+d) = ({onp.array(test_set["y"][69]@A0.T+c0)}, {onp.array(test_set["yd"][69]@A0.T)}, {onp.array(test_set["u"][69]@B0.T+d0)})\n'
...
# !! Check "possible desired solution" !!
RON_evolution_data = onp.load(saved_data_folder/'RON_evolution_N6_withInput.npz')
time_RONsaved = jnp.array(RON_evolution_data['time'])
y_RONsaved = jnp.array(RON_evolution_data['y'])

A_check = jnp.diag(jnp.array([1, 0.3, 0.01, 1, 0.3, 0.01]))
c_check = 0.1*jnp.ones(6)
L_check= L0
D_check = D0
E_check = E0

q_check = y_RONsaved @ jnp.transpose(A_check) + c_check
parameters_check = {
    "th0": jnp.array(jnp.pi/2),
    "L": L_check,
    "r": jnp.array([2e-2, 2e-2]),
...
plt.figure()
plt.plot(time_RONsaved, q_check)
plt.xlabel('t [s]')
plt.ylabel('q')
plt.title(r'$q(t) = A*y_{RON}(t) + c$')
plt.grid(True)
#plt.show()
animate_robot_matplotlib(
        robot = robot_check,
        t_list = time_RONsaved,
        q_list = q_check,
        interval = 1e-2, 
        slider = True,
        animation = False,
...
#     1e-14, 
#     {25*batches_per_epoch: 100, 
#      50*batches_per_epoch: 100, 
#      100*batches_per_epoch: 100, 
#      150*batches_per_epoch: 10, 
#      350*batches_per_epoch: 10, 
#      450*batches_per_epoch: 10}
# )
    ...
    for epoch in tqdm(range(n_iter), 'Training'): # for each epoch...
        ...
        if onp.isnan(loss):
            tqdm.write(
                ...
                f"Last B={onp.array2string(B_print, precision=2, formatter={'float_kind': lambda x: f'{x:.2f}'})} | "
                f"Last d={onp.array2string(d_print, precision=2, formatter={'float_kind': lambda x: f'{x:.2f}'})} | "
        ...
        # perform validation
        L_softplus, D_softplus, E, A_softplus, c, B, d = params_optimiz
        ...
        tqdm.write(
            ...
            f"B={onp.array2string(B_print, precision=2, formatter={'float_kind': lambda x: f'{x:.2f}'})} | "
            f"d={onp.array2string(d_print, precision=2, formatter={'float_kind': lambda x: f'{x:.2f}'})} | "
...
L_softplus_opt, D_softplus_opt, E_opt, A_softplus_opt, c_opt, B_opt, d_opt = params_optimiz_opt
...
plt.figtext(
    ...
    f"E_opt={onp.array(E_opt)}\n"
    f"A_opt={onp.diag(A_opt)}\n"
    f"c_opt={onp.array(c_opt)}\n"
    f"B_opt={onp.array(B_opt.reshape(-1))}\n" 
    f"d_opt={onp.array(d_opt)}", 
    ha="center", va="top"
)
plt.tight_layout()
...
robot_opt = robot.update_params({"L": L_opt, "D": D_opt, "E": E_opt})
...
if show_simulations:    
    ...
    for i, ax in enumerate(axs.flatten()):
        ...
        ax.legend(lines, labels)
    plt.tight_layout()
    plt.savefig(plots_folder/'RONvsPCS_input_after', bbox_inches='tight')
    #plt.show()
...
print(f'Example:\n'
      f'    (y, yd, u) = ({onp.array(test_set["y"][69])}, {onp.array(test_set["yd"][69])}, {onp.array(test_set["u"][69])})\n'
      f'    (q, qd, u_pcs) = (A*y+c, A*yd, B*u+d) = ({onp.array(test_set["y"][69]@A_opt.T+c_opt)}, {onp.array(test_set["yd"][69]@A_opt.T)}, {onp.array(test_set["u"][69]@B_opt.T+d_opt)})\n'
      f'    prediction: {pred_str} = {pred[69]}\n'
      f'    label: {label_str} = {labels[69]}\n'
      f'    |error|: {onp.abs( labels[69] - pred[69] )}'
)
