...
def Loss(
    ...
    controller_updated = mlp_controller.update_params(Phi)
...
dataset = onp.load(dataset_folder/'soft robot optimization/N6_simplified/dataset_m1e5_N6_simplified.npz') # ! 6 decoupled oscillators
...
key, key_mlp = jax.random.split(key, 2)
mlp_sizes = [2*3*n_pcs, 64, 64, 3*n_pcs]                # [input, hidden1, hidden2, output]
mlp_controller = MLP(key=subkey, layer_sizes=mlp_sizes) # initialize MLP feedback control law

L_softplus = InverseSoftplus(L0)
D_softplus = InverseSoftplus(jnp.diag(D0))

phi = (L_softplus, D_softplus)
Phi = tuple(mlp_controller.params) # tuple of tuples with layers: ((W1, b1), (W2, b2), ...)
...
# Phi_opt = mlp_controller.load_params(data_folder/'optimal_data_controller.npz')
# mlp_controller_opt = mlp_controller.update_params(Phi_opt)
