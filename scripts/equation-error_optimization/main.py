...
def Loss(
    ...
    match map_to_train:
        case 'diag' | 'svd':
            ydd_hat_batch = jnp.linalg.solve(A, qdd_batch.T).T # shape (batch_size, 3*n_pcs)
        case 'reconstruction':
...
if show_simulations:
    ...
    fig, axs = plt.subplots(3,1, figsize=(10,6))
    for i in range(n_pcs):
        axs[0].plot(timePCS, u_pcs[:,i], label=f'segment {i+1}')
        axs[0].grid(True)
        axs[0].set_xlabel('t [s]')
        axs[0].set_ylabel(r"$\tau_{be}$ [$N\cdot m^2$]")
        axs[0].set_title('Bending actuation')
        axs[0].legend()
        axs[1].plot(timePCS, u_pcs[:,i+1], label=f'segment {i+1}')
        axs[1].grid(True)
        axs[1].set_xlabel('t [s]')
        axs[1].set_ylabel(r"$\tau_{ax}$ [$N\cdot m$]")
        axs[1].set_title('Axial actuation')
        axs[1].legend()
        axs[2].plot(timePCS, u_pcs[:,i+2], label=f'segment {i+1}')
        axs[2].grid(True)
        axs[2].set_xlabel('t [s]')
