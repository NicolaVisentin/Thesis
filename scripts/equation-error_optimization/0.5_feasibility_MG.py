...
def Loss(
    ...
    data_batch : Dict
        ...
        - **"u"**: Batch of datapoints u. Shape (batch_size, n_inp)
...
@jax.jit
def forward_for_diffrax(mlp: MLP, t: float, z: Array, input_args: Optional[Tuple]=None) -> Array: 
    # extract current input u basing on time
...
dataset = onp.load(dataset_folder/'soft robot optimization/MG_RON_N6_DT0.05_inpscal0.1/dataset_m1e5_N6_DT0.05_inpscal0.1.npz')
...
u = dataset["u"]     # input of the RON oscillators. Shape (m, n_inp)
...
key, subkey = jax.random.split(key)
mlp_sizes = [2*n_ron+n_u, 64, 64, n_ron]  # [input, hidden1, hidden2, output]
...
if show_simulations:
    ...
    RON_evolution_data = onp.load(dataset_folder/'soft robot optimization/MG_RON_N6_DT0.05_inpscal0.1/RON_evolution_N6_DT0.05_inpscal0.1.npz')
    ...
    max_steps = int(1e6)
    term = lambda t, z, args: forward_for_diffrax(mlp_approximator, t, z, args)
    ...
    ax.plot(time_RONsaved, u_RONsaved, 'r', label='u(t)')
    ax.grid(True)
    ax.set_xlabel('t [s]')
    ax.set_ylabel('u')
...
if show_simulations:
    term = lambda t, z, args: forward_for_diffrax(mlp_approximator_opt, t, z, args)
    print('Simulating...')
    start = time.perf_counter()
    solution = diffrax.diffeqsolve(
        terms=diffrax.ODETerm(term),
