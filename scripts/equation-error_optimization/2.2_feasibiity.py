...
def Loss(
    ...
    params_optimiz : Sequence
        ...
        - **p_map**: Parameters of the realNVP.
...
print(f'Example:\n'
      ...
      f'    encoding: (q, qd) = ({onp.array(map.forward(test_set["y"][69]))}, {onp.array(map.forward_with_derivatives(test_set["y"][69], test_set["yd"][69])[1])})\n'
      f'    decoding: (y_hat, yd_hat) = ({onp.array(map.inverse(map.forward(test_set["y"][69])))}, {onp.array(map.inverse_with_derivatives(map.forward(test_set["y"][69]), map.forward_with_derivatives(test_set["y"][69], test_set["yd"][69])[1])[1])})\n'
)
...
if True:
    ...
    # map_opt.save_params() = -> map parameters are not saved/loaded
...
if False:
    ...
    # p_map_opt = -> map parameters are not saved/loaded
    ...
    robot_opt = robot.update_params({"L": L_opt, "D": D_opt, "r": r_opt, "rho": rho_opt, "E": E_opt, "G": G_opt})
    # map_opt = -> map parameters are not saved/loaded
    mlp_controller_opt = mlp_controller.update_params(p_controller_opt)
...
print(f'Example:\n'
      ...
      f'    label: ydd = {labels[69]}\n'
      f'    error: |e| = {onp.abs( labels[69] - pred[69] )}\n\n'
      f'    encoding: (q, qd) = ({onp.array(map_opt.forward(test_set["y"][69]))}, {onp.array(map_opt.forward_with_derivatives(test_set["y"][69], test_set["yd"][69])[1])})\n'
      f'    decoding: (y_hat, yd_hat) = ({onp.array(map_opt.inverse(map_opt.forward(test_set["y"][69])))}, {onp.array(map_opt.inverse_with_derivatives(map_opt.forward(test_set["y"][69]), map_opt.forward_with_derivatives(test_set["y"][69], test_set["yd"][69])[1])[1])})\n'
)
