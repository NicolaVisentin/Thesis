...
class MultiPcsReservoir:
    def __init__(self, n_robots: int, n_pcs: int, map_direct: Callable, map_inverse: Callable, controller: Callable):
        """
        ...
        map_direct : Callable
            Mapping between "virtual" space and physical soft robot space. Must have signature Q, Qd, Qdd = map_direct(y, yd, ydd).
        map_inverse : Callable
            Mapping between physical soft robot space and "virtual" space. Must have signature y, yd, ydd = map_inverse(Q, Qd, Qdd).
        controller : Callable
            (Feedback) control law on the PCS space. Must have signature Tau = controller(Z, u), where Z = [Q, Qd].
        """
        ...
        self.n_pcs = n_pcs
        self.hid_dim = 3 * n_pcs * n_robots

        L = 1e-1 * jnp.ones(n_pcs)
        D = jnp.diag(jnp.tile(jnp.array([5e-6, 5e-3, 5e-3]), n_pcs))
        ...
        self.robots = [PlanarPCS_simple(num_segments=n_pcs, params=parameters, order_gauss=5) for _ in range(n_robots)]
