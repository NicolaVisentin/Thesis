...
experiment_name = 'PLOT' # name of the experiment to save/load
...
load_model_path = saved_data_folder/'equation-error_optimization'/'main_MG_RON'/'B23' # choose the reservoir to load (robot + map + controller)
...
fb_controller_type = 'linear_simple' # 'linear_simple', 'linear_complete', 'tanh_simple', 'tanh_complete', 'mlp'
...
) = load_mackey_glass_data(csvfolder=dataset_folder/'MG', lag=Nl, washout=Nw, train_portion=0.2, val_portion=0.7)
...
ax.plot(full_time[Nw:N_test], full_sequence[Nw:N_test], 'k', label='test sequence')
...
for i, ax in enumerate(axs.flatten()):
    ax.plot(time_ts, y_ts[:,i], 'b', label=r'reservoir $\hat{y}_{i}(t)$')
    ax.plot(time_ts, q_ts[:,i], 'r', label=r'soft robot $q_{i}(t)$')
    ax.grid(True)
    if i == 4 or i == 5:
        ax.set_xlabel('t [s]')
    ax.set_ylabel(r'$\hat{y}$, q')
...
ff_contrib = mlp_ff_controller.forward_batch(test_dataset.reshape((-1,1)))
fb_contrib = mlp_fb_controller.forward_batch(q_ts)
print(ff_contrib.shape, ff_contrib.shape, time_ts.shape)
...
for i, ax in enumerate(axs.flatten()):
    ...
    ax.plot(time_ts, fb_contrib[:,i], 'r', label=r'robot fb actuation $\tau_{fb}(t)$')
    ax.plot(time_ts, ff_contrib[:,i], 'b', label=r'robot ff actuation $\tau_{ff}(t)$')
    if i == 4 or i == 5:
        ax.set_xlabel('t [s]')
    ax.set_ylabel(r'$\tau$')
...
renderer.animate(ts=time_ts, q_ts=q_ts, interval=100, mode="slider")
