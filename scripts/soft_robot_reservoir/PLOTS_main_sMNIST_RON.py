...
# General
example_idx = 3 # if it is an integer i, loads the i-th image from MNIST test set. Otherwise 'black' for black image
train_set_portion = 1 # fraction (or number of images) of the original train set (60 000 images) to use. If 1: full dataset
test_set_portion = 3 # fraction (or number of images) of the original test set (10 000 images) to use. If 1: full dataset
batch_size = 3 # batch size for training and testing. Should be as high as possible, consistently with pc memory and datasets sizes
...
ff_contrib = mlp_ff_controller.forward_batch(jnp.array([image]).reshape((-1,1)))
fb_contrib = mlp_fb_controller.forward_batch(state_pcs_ts)
...
for i, ax in enumerate(axs.flatten()):
    ...
    ax.plot(time_u, ff_contrib[:,i], 'b', label=r'robot ff actuation $\tau_{ff}(t)$')
    if i == 4 or i == 5:
        ax.set_xlabel('t [s]')
    ax.set_ylabel(r'$\tau$')
