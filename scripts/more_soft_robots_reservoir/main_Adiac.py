...
This script:
    ...
    4.  The full architecture (reservoir + output layer) is tested on the test set.

    5.  An example from the Adiac test set, specified in `example_idx`, is loaded and used for inference.

    6.  Another example is taken from the Adiac dataset and used for inference. Dynamics of the reservoir
...
for run, seed in enumerate(seeds):
    ...
    # General
    example_idx = 0 # loads the i-th example from Adiac test set
    batch_size = 30 # batch size for training and testing. Should be as high as possible, consistently with pc memory and datasets sizes
    dt_u = 0.02 # time step for the input u

    # Output layer (scaler + classifier)
    experiment_name = f'Adiac/N6/a_run{n_run}' # name of the experiment to save/load
    ...
    load_model_path = saved_data_folder/'more_soft_robots_optimization'/f'Adiac/N6/default_run{n_run}' # choose the reservoir to load (robots + map + controller)
    ...
    # Load Adiac dataset
    train_set, test_set = load_adiac_data(dataset_folder/'Adiac') # sequnces of shape (n_samples, L, n_u) and labels of shape (n_samples,)

    # Convert to jax
    train_set["series"] = jnp.array(train_set["series"], dtype=jnp.float64)
    train_set["labels"] = jnp.array(train_set["labels"], dtype=jnp.float64)

    test_set["series"] = jnp.array(test_set["series"], dtype=jnp.float64)
    ...
    time_u = jnp.linspace(0, dt_u * (len(train_set["series"][0]) - 1), len(train_set["series"][0])) # define time vector for the input sequence 
    ...
    if train:
        ...
        for i in tqdm(range(len(batch_ids)), 'Model forward'):
            ...
            _, _, _, _, last_states_batch = reservoir_forward(train_batch["series"], time_u, saveat, dt_sim) # shape (batch_size, num_hidden_units)
            last_states.append(last_states_batch)
            labels.append(train_batch["labels"])

        last_states = jnp.concatenate(last_states) # shape (n_samples, num_hidden_units)
        labels = jnp.concatenate(labels) # shape (n_samples,)
    ...
    for i in tqdm(range(len(batch_ids)), 'Model forward'):
        ...
        _, _, _, _, last_states_batch = reservoir_forward(test_batch["series"], time_u, saveat, dt_sim) # shape (batch_size, num_hidden_units)
        last_states.append(last_states_batch)
    ...
    # =====================================================
    # Testing on a single input sequence
    # =====================================================
    print(f'\n--- Testing single example ---')

    # Load sequence to test
    sequence = test_set["series"][example_idx] # shape (L, n_u)
    label = test_set["labels"][example_idx] # shape ()
    ...
    (
        ...
        last_states
    ) = reservoir(sequence, time_u, saveat, dt_sim)
    ...
    plt.title(f'Prediction: {int(pred+1)} | label: {int(label+1)}\n(model test set accuracy: {test_accuracy:.4f})', fontsize=14)
    ...
    for i, ax in enumerate(axs.flatten()):
        ax2 = ax.twinx()
        ax2.plot(time_u, sequence, 'k', alpha=0.3, label=r'reservoir input $u(t)$')
        ax2.set_ylabel(r'$u$')
        ax2.set_ylim([-0.1, 2])
    ...
    # Load another sequence from Adiac dataset
    sequence2 = test_set["series"][example_idx+1] # shape (L, n_u)
    ...
    (
        ...
        actuation_ts2,
        last_states2
    ) = reservoir(sequence2, time_u, saveat, dt_sim)
    ...
    for i, ax in enumerate(axs.flatten()):
        ax2 = ax.twinx()
        ax2.plot(time_u, sequence, 'k', alpha=0.3, label=r'reservoir input $u(t)$ (ex. 1)')
        ax2.plot(time_u, sequence2, 'k--', alpha=0.3, label=r'reservoir input $u(t)$ (ex. 2)')
        ax2.set_ylabel(r'$u$')
        ax2.set_ylim([-0.1, 2])
