...
if False:
    ...
    if do_nomlp_case:
        ...
        """No controller !!"""
        @jax.jit
        def no_control(system_state: SystemState):
            tau = jnp.zeros(int(len(system_state.y) / 2))
            return tau, None
        ...
        with open(plots_folder/test_case/'best_result_parameters.txt', 'w') as file:
            ...
            file.write(f'c = {c_before}\n')
            file.write(f'\nCONTROLLER:\n')
            file.write(f'test |u_i| = {tau_norms_before} (notice this is not actually applied to the system, nor optimized)\n')
            file.write(f'\n\n----------AFTER TRAINING----------\n')
            file.write(f'PCS:\n')
            file.write(f'L = {L_after}\n')
            file.write(f'D = {D_after}\n')
            ...
            file.write(f'c = {c_after}\n')
            file.write(f'\nCONTROLLER:\n')
            file.write(f'test |u_i| = {tau_norms_after} (notice this is not actually applied to the system, nor optimized)\n')
