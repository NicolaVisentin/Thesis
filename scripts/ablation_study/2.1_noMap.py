...
load_case_prefix = 'SAMPLES_NOMAP' # if train_samples is False, choose prefix of the experiment to load (i.e. samples among which looking for the best)
...
def Loss(
    ...
    # DON'T convert variables (NO MAPPING)
    q_batch = y_batch   # shape (batch_size, 3*n_pcs)
    qd_batch = yd_batch # shape (batch_size, 3*n_pcs)
    ...
    # compute loss (compare predictions with labels)
    ydd_hat_batch = qdd_batch # DON'T convert predictions from qdd to ydd
...
if train_samples:
    ...
    s_thresh = 1e-4
    A0 = jnp.tile(jnp.eye(n_ron), (n_samples, 1, 1))
