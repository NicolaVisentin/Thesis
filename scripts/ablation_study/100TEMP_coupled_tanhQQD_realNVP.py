...
def Loss(
    ...
    map : RealNVP
        "Dummy" instance of the map (actual parameters are in params_optimiz).
    ...
    _, _, ydd_hat_batch = map_updated.inverse_with_derivatives_batch(q_batch, qd_batch, qdd_batch) # shape (batch_size, 3*n_pcs)
...
if train_samples:
    ...
    p_robot_before = (L0_raw, D0_raw, r0_raw, rho0_raw, E0_raw, G0_raw)

    # Mapping
    key, key_map, key_init = jax.random.split(key, 3)
    ...
    mlp_controller = MLP(key=key_controller, layer_sizes=mlp_sizes, scale_init=0.0001, last_layer='tanh') # dummy instance
    ...
    # Mapping
    key, key_map = jax.random.split(key)
    n_coupling_layers = 4 # number of coupling layers
    nets_hidden_dim = 32 # dimension of the MLPs (all of them have 2 hidden layers)
    activation_fn_map = 'tanh' # activation function for the MLPs ('tanh' or 'relu')
    scale_t_net = 0.01 # scaling factor on the translation nets initialization (translation MLPs)
    ...
    p_map_before = map.extract_params_from_batch(
        params_batch=all_map_before, 
        idx=idx_best,
    ...
    p_controller_before = mlp_controller.extract_params_from_batch(
...
params_optimiz_before = (p_robot_before, p_map_before, p_controller_before)
...
_, metrics = Loss_vmap(params_optimiz_before, test_set, robot, mlp_controller, map)
...
Loss = jax.jit(partial(Loss, robot=robot, controller=mlp_controller, map=map))
...
_, metrics = Loss_vmap(params_optimiz_after, test_set, robot, mlp_controller, map)
...
map_before = map.update_params(p_map_before)
map_before.save_params(data_folder/'all_data_map_before.npz')

mlp_controller._save_params(p_controller_before, data_folder/'all_data_controller_before.npz')
...
map_after = map.update_params(p_map_after)
map_after.save_params(data_folder/'all_data_map_after.npz')
